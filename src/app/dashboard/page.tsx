...
export default function Dashboard() {
  ...
  // 1. Check for success & Update Database Status
  useEffect(() => {
    const updateStatus = async () => {
      if (searchParams.get('success') === 'true') {
        const { data: { user } } = await supabase.auth.getUser();
        if (user) {
          // Profile status active karein
          await supabase
            .from('profiles')
  ...
  // 2. Initial Data Fetch (Scores + Profile Status)
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-8 text-black font-sans">
      <div className="max-w-2xl mx-auto">
        ...
        {showSuccess && (
          <div className="bg-green-500 text-white px-6 py-4 rounded-2xl mb-6 shadow-lg flex items-center gap-3 animate-in fade-in slide-in-from-top-5 duration-500">
            <span className="text-2xl">🎉</span>
            <div>
              <p className="font-bold text-lg">Payment Successful!</p>
              <p className="opacity-90 text-sm">Your subscription is now active. Thank you for your support!</p>
            </div>
          </div>
        )}
        ...
        <div className="flex justify-between items-start mb-8">
          ...
          <div className="flex flex-col items-end gap-2">
            <button 
              ...
              className="bg-slate-900 text-white px-8 py-2.5 rounded-full font-bold shadow-xl hover:bg-black transition-all disabled:bg-slate-200 disabled:text-slate-400"
            >
              {isPaying ? "Processing..." : subscriptionStatus === 'active' ? "Subscribed ✅" : "Subscribe $10"}
            </button>
            <button onClick={handleLogout} className="text-xs font-bold text-slate-400 hover:text-red-500 transition-colors">Logout Account</button>
          </div>
        </div>

        {/* Charity Impact Card  */}
        <div className="bg-indigo-600 rounded-3xl p-6 mb-8 text-white shadow-2xl shadow-indigo-100 flex justify-between items-center overflow-hidden relative">
        ...
        <form onSubmit={submitScore} className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 mb-8">
          <label className="block mb-3 font-bold text-slate-600 ml-1 text-sm uppercase tracking-wider">Submit Round Score</label>
          <div className="flex gap-4">
            <input 
              type="number" 
              value={score}
              onChange={(e) => setScore(e.target.value)}
              className="bg-slate-50 border-none p-4 rounded-2xl flex-1 focus:ring-2 focus:ring-indigo-400 outline-none transition-all text-lg font-bold"
              placeholder="Enter Stableford (1-45)"
              required
            />
            <button className="bg-indigo-600 hover:bg-indigo-700 text-white px-10 py-4 rounded-2xl font-bold shadow-lg transition-all active:scale-95">
              Save Score
            </button>
          </div>
        </form>
